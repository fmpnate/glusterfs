...
static int
brick_graph_add_io_cache (volgen_graph_t *graph, glusterd_volinfo_t *volinfo,
                         dict_t *set_dict, glusterd_brickinfo_t *brickinfo)
{
        int             ret = -1;
        xlator_t        *xl = NULL;
        ...
        xl = volgen_graph_add (graph, "performance/io-cache", volinfo->volname);
        if (!xl)
                goto out;

        ret = xlator_set_option (xl, "max-file-size", "1GB");
        if (ret)
                goto out;
...
static volgen_brick_xlator_t server_graph_table[] = {
        ...
        {brick_graph_add_io_cache, "io_cache"},
